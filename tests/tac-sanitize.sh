...
# Exercise the tac source with ASan/UBSan in a compatible dynamic Bash.
set -euo pipefail
...
"$CC" -O1 -g -fPIC -shared -Wl,-Bsymbolic -fsanitize=address,undefined \
  ...
  -I"$BT/examples/loadables" loadables/tac.c -o "$d/tac.so"
printf '#!/bin/bash\nexport LD_PRELOAD=%q\nexport TAC_MODULE=%q\nexec %q "$@"\n' \
  "$("$CC" -print-file-name=libasan.so)" "$d/tac.so" "$(realpath "${1:-out/bash}")" > "$d/bash-os"
chmod +x "$d/bash-os"
